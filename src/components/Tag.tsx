...
export const Tag = ({ tag, handleSelectTag, selectedTags, canOpen = true, handleEditTag } : Props) => {
    ...
    return (
        ...
        >
            <section
                className={`w-fit h-fit dark:text-zinc-100 select-none flex items-center justify-center gap-2 transition shadow-lg duration-200 rounded-full py-1 px-3  text-nowrap overflow-ellipsis overflow-hidden  cursor-pointer ${selectedTags.find((selectedTag) => selectedTag.id_tag === tag.id_tag) ? "bg-blue-200 dark:bg-blue-400 hover:bg-blue-300 dark:hover:bg-blue-600 text-black" : "bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-900/50"} transition duration-150`}
            >
                <div 
